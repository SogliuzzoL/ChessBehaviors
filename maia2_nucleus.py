...
of the un-tuned Maia-2 architecture constrained by nucleus (top-p) action space pruning.
"""
...
def evaluate_maia2_nucleus_baseline(
    metadata_path: str = "data/metadata.csv",
    positions_path: str = "data/positions.csv",
    output_predictions_path: str = "data/maia2_nucleus_predictions.csv",
    output_accuracies_path: str = "data/maia2_nucleus_accuracies.csv",
    top_p: float = 0.95,
    device: str = "gpu",
) -> None:
    """Evaluate candidate move prediction accuracy and posterior policy distributions
    using the un-tuned Maia-2 model subjected to nucleus action space pruning.
    ...
    logger.info(
        "Instantiating Maia-2 baseline model wrapper with nucleus pruning (top_p=%.2f)...",
        top_p,
    )
...
if __name__ == "__main__":
    evaluate_maia2_nucleus_baseline()
