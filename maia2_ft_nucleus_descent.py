"""
Cross-validation evaluation pipeline for evaluating fine-tuned Maia-2 model accuracy
and move policy distributions under nucleus pruning and descent search optimization.
"""
...
def evaluate_maia2_ft_nucleus_descent(
    ...
    """Evaluate fine-tuned Maia-2 model accuracy and move policy distributions using
    subject-level fine-tuning, nucleus pruning, and descent search optimization
    ...
    Args:
        ...
        timeout (float, optional): Maximum search execution time in seconds per board state.
            Defaults to 1.0.
        top_p (float, optional): Probability threshold cumulative limit for nucleus pruning.
            Defaults to 0.95.
    ...
    logger.info(
        "Loading subject metadata and loading positional observation records..."
    )
    players = getPlayers(metadata_path)
    players_dict: dict[str, int] = createPlayerDict(players)
    positions = pl.read_csv(positions_path)

    # Define nucleus pruning function over candidate move probabilities
    prune_fn = lambda moves: nucleus_prune(moves, top_p=top_p)

    logger.info(
        "Instantiating fine-tuned Maia-2 architecture with nucleus pruning (top_p=%.2f)...",
        top_p,
    )
    ...
    for player_name, player_index in player_pbar:
        ...
        for fold_idx, test_games in fold_pbar:
            ...
            # Reset and fit player-specific embeddings on training game partition
    ...
    output_predictions_path = (
        f"data/maia2_ft_nucleus_descent_{max_iterations}_predictions.csv"
    )
    output_accuracies_path = (
        f"data/maia2_ft_nucleus_descent_{max_iterations}_accuracies.csv"
    )
...
if __name__ == "__main__":
    evaluate_maia2_ft_nucleus_descent()
