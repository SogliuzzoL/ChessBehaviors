...
of the un-tuned Maia-2 model integrated with Monte Carlo Tree Search (MCTS) optimization.
"""
...
def evaluate_maia2_mcts_baseline(
    ...
    """Evaluate candidate move decision accuracy and posterior policy distributions
    using the un-tuned Maia-2 architecture combined with Monte Carlo Tree Search (MCTS)
    across human subject cohorts.
    ...
    for player_name, player_index in player_pbar:
        ...
        for row in position_pbar:
            ...
            # Determine mode candidate move corresponding to maximum posterior probability
            predicted_move = (
    ...
    output_predictions_path = f"data/maia2_mcts_{max_iterations}_predictions.csv"
    output_accuracies_path = f"data/maia2_mcts_{max_iterations}_accuracies.csv"
    general_accuracies_path = "data/maia2_mcts_accuracies.csv"
    ...
    if accuracies:
        ...
        # Duplicate un-suffixed record for downstream comparative integration
