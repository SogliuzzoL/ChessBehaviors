...
if __name__ == "__main__":
    ...
    mcts_agent = MCTSModelWrapper(base_model=maia2_model, max_iterations=N, timeout=1.0)
    ...
    predictions_df.to_csv(f"data/maia2_mcts_{N}_predictions.csv", index=False)

    accuracies_df = pl.DataFrame(accuracies)
    accuracies_df.write_csv(f"data/maia2_mcts_{N}_accuracies.csv")

    # Copie sans suffixe pour faciliter l'intégration dans les scripts de comparaison
    accuracies_df.write_csv("data/maia2_mcts_accuracies.csv")
